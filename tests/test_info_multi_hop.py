...
def _template_test_mesh_network_info(mesh_network:GraphFixtureValue, aio_benchmark):
    ...
    info:InfoPacket = aio_benchmark(get_info, client_id=reciever_client.client_id, rounds=100, iterations=1)

    assert info is not None

    assert {con.to_tuple() for con in info.connections.values()} == connection_set

def test_send_info_graph(mesh_network_graph:GraphFixtureValue, aio_benchmark):
    _template_test_mesh_network_info(mesh_network_graph, aio_benchmark)

def test_send_info_linked_list(mesh_network_linked_list:GraphFixtureValue, aio_benchmark):
